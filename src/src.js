...
function createSnake() {
  function generateSnake() {
  	...
  	let posY = Math.round(Math.random()*25 + 7);
  	return [posX, posY];
  }
...
window.addEventListener('load', function(){
  ...
  touchsurface.addEventListener('touchend', function(e){
    ...
    if ( (elapsedTime <= 111) && touchStep ){
      touchClick = true;
      touchStep = false;
    }
    if ( (elapsedTime <= 666) && touchStep ) {
      if (Math.abs(touchObject.pageY - startY) < 66) {
        if ( (touchObject.pageX-startX) > 66) { touch = 'right'; }
        if ( (touchObject.pageX-startX) < -66) { touch = 'left' }
      }
      if (Math.abs(touchObject.pageX - startX) < 66) {
        if ( (touchObject.pageY-startY) > 66) { touch = 'down'; }
        if ( (touchObject.pageY-startY) < -66) { touch = 'up' }
      }
      touchStep = false;
    }
    e.preventDefault();
